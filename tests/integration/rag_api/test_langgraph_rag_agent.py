...
# Configure logging
logger = logging.getLogger("tests.integration.rag_api.test_langgraph_rag_agent")

@pytest.mark.asyncio
async def test_langgraph_rag_agent_initialization(mock_vector_store, mock_ollama_client):
...
@pytest.mark.asyncio
async def test_langgraph_rag_agent_query(mock_langgraph_rag_agent):
    ...
    response = await mock_langgraph_rag_agent.query(
        query=query,
        stream=False
    )
