...
async def upload_document(
    ...
    try:
        ...
        try:
            ...
            query = text("""
                INSERT INTO documents (id, filename, folder, uploaded, processing_status)
                VALUES (:id, :filename, :folder, :uploaded, :status)
            """)
...
async def get_all_tags(
    ...
    """
    try:
        # Query all tags from the database using SQLAlchemy 2.0 style
        result = await db.execute(text("SELECT name FROM tags ORDER BY name"))
        tags = result.all()
...
async def get_all_folders(
    ...
    """
    try:
        # Query all folders from the database using SQLAlchemy 2.0 style
        result = await db.execute(text("SELECT path FROM folders ORDER BY path"))
        folders = result.all()
    ...
    try:
        ...
        try:
            ...
            for doc_id in document_ids:
                ...
                from sqlalchemy import text
                query = text("SELECT id FROM documents WHERE id = :id")
